...
class BudgetedCausalAttention(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, n_kv_heads: Optional[int]=None, dropout: float=0.0, span_min: int=8, span_temp: float=4.0, sliding_window: Optional[int]=None, quantize_kv: bool=False):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, signal: Optional[ControllerSignal]=None, hard: bool=False, use_cache: bool=False, past_key_value: Optional[tuple]=None, freqs_cis: Optional[torch.Tensor]=None) -> torch.Tensor | tuple[torch.Tensor, Optional[tuple]]:
        B, T_q, d = x.shape
        ...
        g = signal.g if signal is not None else torch.ones(B, T_q, device=x.device, dtype=x.dtype)
        ...
        log_gate = torch.log(gate.clamp_min(1e-09)).unsqueeze(1)
...
if __name__ == '__main__':
    ...
    x = torch.randn(B, T, d)
    lo = ControllerSignal(g=torch.zeros(B, T), c=torch.zeros(B, T), logits=torch.zeros(B, T, 2))
    hi = ControllerSignal(g=torch.ones(B, T), c=torch.ones(B, T), logits=torch.zeros(B, T, 2))
    out_lo = att(x, lo)[0]
    out_hi = att(x, hi)[0]
    diff = (out_lo - out_hi).abs().mean().item()
    print(f'attn_budget smoke OK | out {tuple(out_hi.shape)} | lo-vs-hi span mean|delta|={diff:.4f} (should be > 0)')
    assert diff > 0.0001, 'span budget had no effect'