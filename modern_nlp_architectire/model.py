...
class DenseDecoder(nn.Module):
    ...
    def __init__(self, cfg: ModelConfig):
        ...
        self.blocks = nn.ModuleList([TransformerBlock(cfg.d_model, cfg.n_heads, cfg.d_ff, cfg.dropout, cfg.norm, attn_budget=cfg.attn_budget, span_min=cfg.span_min, span_temp=cfg.span_temp, residual=cfg.residual, moe=cfg.moe, moe_experts=cfg.moe_experts, moe_k_max=cfg.moe_k_max, moe_expert_ff=cfg.moe_expert_ff) for _ in range(cfg.n_layers)])
        self.ln_f = nn.LayerNorm(cfg.d_model)
        self.lm_head = nn.Linear(cfg.d_model, cfg.vocab_size, bias=False)
        if cfg.tie_embeddings:
            self.lm_head.weight = self.tok_emb.weight
        self.mtp_head = nn.Linear(cfg.d_model, cfg.vocab_size, bias=False) if cfg.mtp else None
        ...
        if cfg.dyn_patch:
            self.patcher = DynamicPatcher(cfg.d_model, cfg.patch_hidden, cfg.patch_ctx_weight, cfg.patch_use_signal)
        ...
        if cfg.latent_ponder:
            self.ponder = LatentPonder(cfg.d_model, cfg.n_heads, cfg.d_ff, cfg.dropout, cfg.norm, cfg.attn_budget, cfg.span_min, cfg.span_temp, cfg.residual, cfg.ponder_max_steps, cfg.ponder_min_steps, cfg.ponder_prior_lambda, cfg.ponder_g_bias)
    ...
    def forward(self, x: torch.Tensor, gt_difficulty: Optional[torch.Tensor]=None, patch_aux_weight: float=0.0, skip_ponder: bool=False) -> torch.Tensor:
        B, T = x.shape
