...
@triton.jit
def scatter_to_experts_kernel(tokens_ptr, expert_idx_ptr, weights_ptr, out_tokens_ptr, expert_counts_ptr, d_model: int, top_k: int, stride_tok_b, stride_tok_d, stride_out_e, stride_out_c, stride_out_d, BLOCK_SIZE_D: int, BLOCK_SIZE_N: int):
    if tl is None:
        return
    pid = tl.program_id(axis=0)

@triton.jit
def gather_from_experts_kernel(expert_out_ptr, expert_idx_ptr, weights_ptr, out_tokens_ptr, d_model: int, top_k: int, stride_exp_e, stride_exp_c, stride_exp_d, stride_out_b, stride_out_d, BLOCK_SIZE_D: int):
    if tl is None:
        return
    pid = tl.program_id(axis=0)
