...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, signal: Optional[ControllerSignal]=None, use_cache: bool=False, past_key_value: Optional[tuple[torch.Tensor, torch.Tensor]]=None, freqs_cis: Optional[torch.Tensor]=None) -> tuple[torch.Tensor, Optional[tuple[torch.Tensor, torch.Tensor]]]:
        B, T_q, d = x.shape
        ...
        out = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, dropout_p=self.drop.p if self.training else 0.0, is_causal=is_causal)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, n_kv_heads: int, d_ff: int, *, dropout: float=0.0, norm: str='pre', attn_budget: bool=False, span_min: int=8, span_temp: float=4.0, residual: str='vanilla', moe: bool=False, moe_experts: int=4, moe_k_max: int=2, moe_expert_ff: int=0, sliding_window: Optional[int]=None, quantize_kv: bool=False):
        super().__init__()
        self.norm = norm
        self.ln1 = RMSNorm(d_model)
        self.ln2 = RMSNorm(d_model)
        if attn_budget:
            self.attn: nn.Module = BudgetedCausalAttention(d_model, n_heads, n_kv_heads=n_kv_heads, dropout=dropout, span_min=span_min, span_temp=span_temp, sliding_window=sliding_window, quantize_kv=quantize_kv)
        else:
            self.attn = CausalSelfAttention(d_model, n_heads, n_kv_heads, dropout, sliding_window=sliding_window, quantize_kv=quantize_kv)
        self.is_moe = moe
        if moe:
            self.mlp: nn.Module = MoELayer(d_model, moe_expert_ff or d_ff, moe_experts, moe_k_max)
        else:
            self.mlp = MLP(d_model, d_ff, dropout)
        self.res_attn = ResidualScaler(d_model, residual)
        self.res_mlp = ResidualScaler(d_model, residual)

    def forward(self, x: torch.Tensor, signal: Optional[ControllerSignal]=None, use_cache: bool=False, past_key_value: Optional[tuple[torch.Tensor, torch.Tensor]]=None, freqs_cis: Optional[torch.Tensor]=None):
        present_key_value = None
        if self.norm == 'pre':
...
if __name__ == '__main__':
    ...
    print(f'block smoke OK | out {tuple(out_sig.shape)} | signal inert: {torch.allclose(out_none, out_sig)}')