...
def greedy_decode(model, prompt: List[int], n_new: int, device) -> DecodeStats:
    ...
    return DecodeStats(seq, n_verify=n_new, n_draft=0, n_generated=n_new, acceptance=0.0, matches_greedy=True)

@torch.no_grad()
def speculative_decode(model, prompt: List[int], n_new: int, device, gamma: int=4, g_threshold: float=0.6) -> DecodeStats:
    model.eval()
    seq = list(prompt)
    ...
    return DecodeStats(seq, n_verify=n_verify, n_draft=n_draft, n_generated=n_new, acceptance=accepted / max(1, drafted), matches_greedy=seq == greedy)
if __name__ == '__main__':
    ...
    print(f'decode smoke OK | matches_greedy={spec.matches_greedy} | verify={spec.n_verify} vs greedy={spec.n_generated} (speedup {spec.n_generated / spec.n_verify:.2f}x) | acceptance={spec.acceptance:.2f}')
