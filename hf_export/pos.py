...
class LearnedPositional(nn.Module):
    ...
    def forward(self, T: int, device: torch.device) -> torch.Tensor:
        ...
        return self.emb(torch.arange(T, device=device))          # (T, d)
...
class NoPE(nn.Module):
    ...
    def forward(self, T: int, device: torch.device) -> torch.Tensor:
        return torch.zeros((), device=device)                    # scalar 0 (broadcasts)
...
if __name__ == "__main__":
    # Smoke test: NoPE is length-agnostic; learned respects max_seq_len.
    d = 32
    nope = build_positional("nope", 128, d)
    learned = build_positional("learned", 128, d)
    dev = torch.device("cpu")
    assert nope(9999, dev).shape == ()                      # any length OK
