...
@dataclass
class MemInfo:
    read_gate_mean: float        # mean g-gate applied to reads (all tokens)
    write_mass: float            # total gated write mass this step
    slot_usage: torch.Tensor     # (M,) mean read attention per slot
...
class GatedKVMemory(nn.Module):
    ...
    def __init__(self, d_model: int, *, n_slots: int = 32, write_ema: float = 0.99):
        ...
        # Persistent write buffer (not a grad parameter): added to values on read.
    ...
    def forward(self, h: torch.Tensor,
        ...
        g = (signal.g if signal is not None
             else torch.ones(B, T, device=h.device, dtype=h.dtype))   # (B,T)

        vals = self.values + self.write_buf                          # (M, d)
        att = F.softmax((h @ self.keys.t()) * self.scale, dim=-1)     # (B,T,M)
        read = att @ vals                                            # (B,T,d)
        h_out = h + g.unsqueeze(-1) * self.read_proj(read)           # gated read

        # g-gated cross-step write (EMA into buffer; no grad, no leak).
        if self.training:
            with torch.no_grad():
                wgate = g.reshape(-1, 1)                             # (N,1)
                hn = h.reshape(-1, d)                                # (N,d)
                wa = F.softmax((hn @ self.keys.t()) * self.scale, dim=-1)  # (N,M)
                # slot update = sum_t g_t * wa_{t,slot} * h_t  (normalized)
                upd = (wa * wgate).t() @ hn                          # (M,d)
...
if __name__ == "__main__":
    # Smoke test: at g=0 memory is inert; at g=1 it perturbs the residual.
    torch.manual_seed(0)
    B, T, d = 2, 16, 48
