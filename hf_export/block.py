...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        # signal is accepted but IGNORED here until #1 attn_budget (Stage 4).
        B, T, d = x.shape
...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
                signal: Optional[ControllerSignal] = None) -> torch.Tensor:
        return self.drop(self.fc2(F.gelu(self.fc1(x))))       # signal ignored (dense)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int, *,
        ...
        if attn_budget:                                   # #1 g_t-driven span
        ...
        self.is_moe = moe
        if moe:                                           # #4 difficulty-scaled MoE
            self.mlp: nn.Module = MoELayer(
                d_model, moe_expert_ff or d_ff, moe_experts, moe_k_max)
        else:
            self.mlp = MLP(d_model, d_ff, dropout)
        self.res_attn = ResidualScaler(d_model, residual)  # #7 structured residual
...
if __name__ == "__main__":
    # Smoke test: signal-threaded forward is shape-preserving and inert.
    torch.manual_seed(0)
    ...
    # Inert: passing the signal must not change the output.
