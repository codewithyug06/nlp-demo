...
EWC-style elastic consolidation with the PANOPTES twist: the per-parameter
...
def compute_importance(model, make_fn, spec, device, n_batches: int = 16,
    ...
    for _ in range(n_batches):
        ...
        mask = b.difficulty > 0
        nll = F.cross_entropy(logits[mask], b.y[mask], reduction="none")   # (N,)
...
if __name__ == "__main__":
    # Smoke test: importance is non-negative; penalty is 0 at the anchor, >0 away.
    import sys, os
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from panoptes.model import build_model
    ...
    p0 = cons.penalty(model).item()                       # at anchor -> 0
    with torch.no_grad():
        next(model.parameters()).add_(0.1)
    p1 = cons.penalty(model).item()                       # moved away -> > 0
